...
function createTopic() {
    if (!forumTextArea || !forumTopicNameArea || !fid) {
        return;
    }
    
    let form = new FormData();
    form.append("fid", fid);
    form.append("fname", forumTopicNameArea.value);
    form.append("post_content", forumTextArea.value);
    form.append("images", JSON.stringify(images));
    
    requestForm("/php/forumCreateTopic.php", form, function(data) {
        if (data === "success") {
            location.href = "/forum.php?fid=" + fid;
        } else if (typeof data === "string") {
            alert(data);
        } else {
            alert("Ошибка. Сервер вернул неожиданный ответ. Обратитесь к администратору");
...
const forumTopicNameArea = document.getElementById("forumTopicNameArea");
...
const forumButtonCreate = document.getElementById("forumButtonCreate");

if (forumButtonCreate) {
    forumButtonCreate.addEventListener("click", createTopic);
}
